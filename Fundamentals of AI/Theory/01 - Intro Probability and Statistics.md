Probability is a very important Machine Learning tool because it allows to summarize uncertainty from various sources, such as:
- Uncertain inputs: missing or noisy data
- Uncertain knowledge: stochastic effects or incomplete knowledge of the causality of the domain
...
## Probability
The probability $P(A)$ that event $A$ happens, is a function that maps the event $A$ onto the interval $[0, 1]$. $P(A)$ is also called the probability measure of $A$.
...
Probability that $X$ takes some value: 
...
Usually $B$ is an interval of the type $[a, b]$ so:
$$P(a \le X \le b) = \int_a^b f(t)dt$$

## Probability Density Function
From this definition we can say that a probability density function is a nonnegative function that integrates to one.
616
...
Two or more variables may **interact**. So, the probability of one variable to take a certain value depends on what values the others variables are taking: 
$$f_{X, Y} (x, y) = P(X = x \text{ and } Y = y)$$
![[01 - Intro Probability and Statistics-1786634858079.webp|618]]

## Marginal Distribution
Marginal distributions are sub-tables which eliminate variables, **marginalization** means combine collapsed rows using the addiction.
![[01 - Intro Probability and Statistics-1786640198992.webp|559]]
...
## Independence
$Y$ and $X$ are independent if they don't contain information about each others:
...
# Bayes' Rule
The **Bayes' Rule** is one of the most fundamental results in probability theory, and it connects prior knowledge with observed evidence. It is derived directly from the definition of conditional probability: 
$$P(A|B) = \frac{P(B|A) \cdot P(A)}{P(B)}$$ 
In the context of machine learning and inference, Bayes' rule is typically written as:
...
A common application of Bayes' rule is **classification**. Given a class label $y$ and a set of attributes (**features**) $\mathbf{x} = (x_1, x_2, \ldots, x_n)$, we want to compute:
...
This leads to the **Naïve Bayes classifier**:
...
- Despite the conditional independence assumption (which rarely holds in practice), Naïve Bayes shows good performance in many application domains.
- Best used when a moderate or large training set is available and instances are represented by a large number of attributes.

### Laplacian Correction
An issue arises when an attribute value never appears together with a certain class label in the training set. For example:
$$P(\text{cold} | \text{yes}) = 0 \implies P(\text{yes} | \text{cold}, \ldots) = 0$$
This is problematic because a single zero probability wipes out the entire product. The **Laplacian correction** (also called Laplace smoothing) fixes this by adding one or more virtual samples for each combination: 
...
The flips are assumed to be **independently and identically distributed** (i.i.d.):
- **Independent**: each flip does not affect the others
- **Identically distributed**: each flip follows the same Bernoulli distribution

## Maximum Likelihood Estimation (MLE)
**Maximum likelihood estimation** chooses the parameter $\theta$ that maximizes the probability of the observed data **D**:
...
**Example --- coin flipping**: if we observe 3 heads and 2 tails, the MLE gives: 
$$\hat{\theta} = \frac{3}{5}$$
which is simply the *frequency of heads* --- exactly what one would intuitively expect.

### MLE for Gaussian Mean and Variance
Given $m$ i.i.d. samples $\{x^{(1)}, \ldots, x^{(m)}\}$ drawn from $\mathcal{N}_x(\mu, \sigma)$, the MLE estimates are:
$$p(x | \mu , \sigma) = \frac{1}{\sqrt{2 \pi \sigma^2}} e^{\left(- \frac{(x - \mu)^2}{2 \sigma^2}\right)} = \mathcal{N}_x(\mu, \sigma)$$
![[01 - Intro Probability and Statistics-1786640478732.webp|439]]

Choose $\theta=(\mu, \sigma^2)$ that maximize the probability of observed data: 
...
## Bias
The bias of an estimator $\hat{\theta}_m = g(x^{(1)}, ..., x^{(m)})$ for a parameter $\theta$:
$$\text{Bias}(\hat{\theta}) = \mathbb{E}[\hat{\theta}] - \theta$$ 
An estimator is called unbiased if $\text{Bias}(\hat{\theta}) = 0$.
...
$$\text{SE}(\hat{\mu}) = \frac{\sigma}{\sqrt{m}}$$ 
where $\sigma$ is the true standard deviation and $m$ is the dataset size. In practice, $\sigma$ is estimated from the data.
...
MLE ignores any prior knowledge we may have about $\theta$. MAP estimation incorporates a prior $P(\theta)$ via Bayes' rule:
...
A practical issue with MAP: the result depends on the choice of prior, and can perform poorly when the number of samples is small.
542

---

# Discriminative vs. Generative Learning
Many supervised learning methods can be viewed as estimating $P(X, Y)$. They generally fall into two categories:
...
As the number of i.i.d. samples $m \to \infty$, the sample mean converges to the true expectation:
...
Let $x^{(1)}, \ldots, x^{(m)}$ be i.i.d. samples with mean $\mu$ and variance $\sigma^2$. Then the sample mean $\bar{X}_m = \frac{1}{m}\sum_i x^{(i)}$ satisfies:
$$\sqrt{m} \cdot \frac{\bar{X}_m - \mu}{\sigma} \xrightarrow{d} \mathcal{N}(0, 1) \quad \text{as } m \to \infty$$ 
In other words, regardless of the original distribution, the sample mean is approximately normally distributed for large $m$. This is the theoretical foundation for confidence intervals and hypothesis testing.
507